...
export default function CustomDataPage() {
  ...
  // Query hooks
  const { data: customData, isLoading, isError, error, refetch: refetchCustomData } = useCustomData();
...
function CustomDataCard({
  ...
  const { data: linkedItems, refetch: refetchLinked } = useLinkedItems('custom_data', `${item.category}:${item.key}`);
        ...
        <div className="space-y-4">
          ...
          {showLinkModal && (
            <LinkItemsModal
              ...
              currentItemId={`${item.category}:${item.key}`}
              onLinkCreated={() => {
                refetchLinked();
