...
def normalize_workspace_id(workspace_id: str) -> str:
    """
    Normalize workspace_id to ensure case-insensitive matching.
    Converts Windows drive letters to lowercase: C:\ -> c:/
...
def handle_get_product_context(args: models.GetContextArgs) -> Dict[str, Any]:
    ...
    try:
        ...
        context_model = db.get_product_context(normalized_workspace_id)
...
def handle_update_product_context(args: models.UpdateContextArgs) -> Dict[str, Any]:
    ...
    try:
        ...
        db.update_product_context(normalized_workspace_id, args)
...
def handle_log_decision(args: models.LogDecisionArgs) -> Dict[str, Any]:
    ...
    try:
        ...
        logged_decision = db.log_decision(normalized_workspace_id, decision_to_log)
...
def handle_search_progress_fts(args: models.SearchProgressArgs) -> List[Dict[str, Any]]:
    """
    Handles the 'search_progress_fts' MCP tool.
    Assumes 'args' is an already validated Pydantic model instance.
    ...
    try:
        ...
        return [p.model_dump(mode='json') for p in progress_entries]
    except DatabaseError as e:
        raise ContextPortalError(f"Database error searching progress entries: {e}")
    except Exception as e:
        log.exception(f"Unexpected error in search_progress_fts for workspace {args.workspace_id}")
        raise ContextPortalError(f"Unexpected error in search_progress_fts: {e}")

def handle_search_system_patterns_fts(args: models.SearchSystemPatternsArgs) -> List[Dict[str, Any]]:
    """
    Handles the 'search_system_patterns_fts' MCP tool.
    Assumes 'args' is an already validated Pydantic model instance.
    ...
    try:
        ...
        raise ContextPortalError(f"Database error searching system patterns: {e}")
    except Exception as e:
        log.exception(f"Unexpected error in search_system_patterns_fts for workspace {args.workspace_id}")
        raise ContextPortalError(f"Unexpected error in search_system_patterns_fts: {e}")

def handle_search_context_fts(args: models.SearchContextArgs) -> List[Dict[str, Any]]:
    """
    Handles the 'search_context_fts' MCP tool.
    Assumes 'args' is an already validated Pydantic model instance.
    ...
    try:
        ...
        raise ContextPortalError(f"Database error searching contexts: {e}")
    except Exception as e:
        log.exception(f"Unexpected error in search_context_fts for workspace {args.workspace_id}")
        raise ContextPortalError(f"Unexpected error in search_context_fts: {e}")
...
def handle_get_active_context(args: models.GetContextArgs) -> Dict[str, Any]:
    ...
    try:
        normalized_workspace_id = normalize_workspace_id(args.workspace_id)
        context_model = db.get_active_context(normalized_workspace_id)
        return context_model.content
    except DatabaseError as e:
...
def handle_update_active_context(args: models.UpdateContextArgs) -> Dict[str, Any]:
    ...
    try:
        ...
        db.update_active_context(normalized_workspace_id, args)
...
def handle_update_decision(args: models.UpdateDecisionArgs) -> Dict[str, Any]:
    """
    Handles the 'update_decision' MCP tool.
    Assumes 'args' is an already validated Pydantic model instance.
    Returns a status message dictionary.
    """
    try:
        ...
        if updated:
            # --- Update Vector Store ---
            # Re-embedding on update requires fetching the full, updated entry from the DB
            # to get the complete summary, rationale, and implementation_details for the vector.
            # This requires a db.get_decision_by_id function, which is not yet implemented.
            # For now, we will skip re-embedding on update and log a warning.
            # A future enhancement would be to implement db.get_decision_by_id
            # and then call vector_store_service.upsert_item_embedding here.
            log.warning(f"Vector store update skipped for decision ID {args.decision_id} on update. Requires db.get_decision_by_id for accurate re-embedding.")
            # --- End Update Vector Store ---

            return {"status": "success", "message": f"Decision ID {args.decision_id} updated successfully."}
        else:
            return {"status": "success", "message": f"Decision ID {args.decision_id} not found for update."}
    except ValueError as e: # Catch validation errors from the handler/db call
         raise ToolArgumentError(str(e))
    except DatabaseError as e:
        raise ContextPortalError(f"Database error updating decision ID {args.decision_id}: {e}")
    except Exception as e:
        log.exception(f"Unexpected error in handle_update_decision for workspace {args.workspace_id}, ID {args.decision_id}")
        raise ContextPortalError(f"Unexpected error updating decision: {e}")
...
def handle_log_custom_data(args: models.LogCustomDataArgs) -> Dict[str, Any]:
    ...
    try:
        ...
        logged_data = db.log_custom_data(normalized_workspace_id, data_to_log)
...
def handle_get_custom_data(args: models.GetCustomDataArgs) -> List[Dict[str, Any]]:
    ...
    try:
        normalized_workspace_id = normalize_workspace_id(args.workspace_id)
        data_list = db.get_custom_data(normalized_workspace_id, category=args.category, key=args.key)
...
def handle_get_all_custom_data_by_id_desc(args: models.GetAllCustomDataByIdDescArgs) -> List[Dict[str, Any]]:
    """
    Handles the 'get_all_custom_data_by_id_desc' MCP tool.
    Retrieves all custom data entries sorted by ID descending for UI display.
    """
    try:
        normalized_workspace_id = normalize_workspace_id(args.workspace_id)
        data_list = db.get_all_custom_data_by_id_desc(normalized_workspace_id, limit=args.limit)
        return [d.model_dump(mode='json') for d in data_list]
    except DatabaseError as e:
        raise ContextPortalError(f"Database error getting custom data by ID desc: {e}")
    except Exception as e:
        log.exception(f"Unexpected error in get_all_custom_data_by_id_desc for workspace {args.workspace_id}")
        raise ContextPortalError(f"Unexpected error in get_all_custom_data_by_id_desc: {e}")
...
def handle_link_conport_items(args: models.LinkConportItemsArgs) -> Dict[str, Any]:
    ...
    try:
        ...
        logged_link = db.log_context_link(normalized_workspace_id, link_to_create)
...
def handle_update_link(args: models.UpdateLinkArgs) -> Dict[str, Any]:
    """
    Handles the 'update_link' MCP tool.
    ...
    try:
        ...
        if updated_link:
            return {
                "status": "success",
                "message": f"Link ID {args.link_id} updated successfully.",
                #"link": updated_link.model_dump(mode='json') removed because this is a boolean, not the link itself.
            }
        else:
            return {"status": "success", "message": f"Link ID {args.link_id} not found for update."}
    except ValueError as e:
        raise ToolArgumentError(str(e))
    except DatabaseError as e:
        raise ContextPortalError(f"Database error updating link ID {args.link_id}: {e}")
    except Exception as e:
        log.exception(f"Unexpected error in handle_update_link for workspace {args.workspace_id}, link ID {args.link_id}")
        raise ContextPortalError(f"Unexpected error updating link: {e}")

def handle_delete_link_by_id(args: models.DeleteLinkByIdArgs) -> Dict[str, Any]:
    """
    Handles the 'delete_link_by_id' MCP tool.
    ...
    try:
        ...
        if deleted_from_db:
            ...
            return {"status": "success", "message": f"Link ID {args.link_id} not found in database."}
    except DatabaseError as e:
        raise ContextPortalError(f"Database error deleting link ID {args.link_id}: {e}")
    except Exception as e:
        log.exception(f"Unexpected error in handle_delete_link_by_id for workspace {args.workspace_id}, link ID {args.link_id}")
        raise ContextPortalError(f"Unexpected error deleting link: {e}")

def handle_get_items_by_references(args: models.GetItemsByReferencesArgs) -> List[Dict[str, Any]]:
    """
    Handles the 'get_items_by_references' MCP tool.
    Enhanced bulk item retrieval with dual interface and comprehensive error handling.
    """
    try:
        normalized_workspace_id = normalize_workspace_id(args.workspace_id)
        # Call the enhanced database function directly with both interface options
        results = db.get_items_by_references(
            workspace_id=normalized_workspace_id,
            references=args.references,
            linked_items_result=args.linked_items_result
        )
        return results
        
    except ValueError as e:
        # Catch validation errors from database function (mutual exclusivity, etc.)
        raise ToolArgumentError(str(e))
    except DatabaseError as e:
        raise ContextPortalError(f"Database error retrieving items by references: {e}")
    except Exception as e:
        log.exception(f"Unexpected error in get_items_by_references for workspace {args.workspace_id}")
        raise ContextPortalError(f"Unexpected error retrieving items by references: {e}")
