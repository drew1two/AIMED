...
export function GraphCanvas({
  ...
  // Ref for linkMode to avoid stale closures in event handlers
  const linkModeRef = useRef(linkMode);
  const linkCreationStateRef = useRef(linkCreationState);
  
  // Context menu state for edge interactions and link creation (supports multi-edge selection)
  ...
  // Legacy LinkCreationMenu state (kept for compatibility but unused)
...
const handleSaveContextEdit = useCallback((description?: string) => {
  try {
    if (contextMenu.linkId && onLinkEdit) {
      const nextType = editRelType || contextMenu.linkData?.relationship_type || 'relates_to';
      onLinkEdit(contextMenu.linkId, nextType, description);
    }
  } finally {
    setContextMenu(prev => ({ ...prev, visible: false, linkData: null, linkId: null, edges: null, linkMode: false, sourceNodeId: null, targetNodeId: null }));
  }
...
// Handle right-click link creation
const handleCreateLinkFromContextMenu = useCallback((relationshipType: string, description?: string) => {
  if (onLinkCreate && contextMenu.sourceNodeId && contextMenu.targetNodeId) {
    try {
      debugLog('🔗 LINK_CREATION_FIX: Creating link via right-click context menu:', {
        source: contextMenu.sourceNodeId,
        target: contextMenu.targetNodeId,
        relationshipType,
        description
      });

      // Invoke server create with description
      onLinkCreate(
        contextMenu.sourceNodeId,
        contextMenu.targetNodeId,
        relationshipType,
        description || `Link from ${contextMenu.sourceNodeId} to ${contextMenu.targetNodeId}`
      );
      ...
      console.error('[Graph] Right-click link creation error', e);
    }
  }
  
  // Reset context menu and link creation state
  setContextMenu(prev => ({
    ...prev,
    visible: false,
    linkMode: false,
  ...
  setLinkCreationState({ sourceNodeId: null, isCreatingLink: false });
}, [onLinkCreate, contextMenu.sourceNodeId, contextMenu.targetNodeId]);
...
useEffect(() => {
  ...
  useEffect(() => {
    debugLog('🔗 LINK_CREATION_FIX: GraphCanvas props updated:', {
      selectedNodeId,
      linkMode,
      linkCreationState,
      searchMatchingIds: searchMatchingIds.size
    });
...
useEffect(() => {
  ...
  const handleCanvasClick = useCallback((event: MouseEvent) => {
    ...
    debugLog('🔗 LINK_CREATION_FIX: Canvas click - Initial state:', {
      linkMode,
      linkCreationState,
      currentLinkCreateUI: linkCreateUI,
      pos
    });
    ...
    debugLog('🔗 LINK_CREATION_FIX: Hit test results:', {
      clickedNode: clickedNode?.id ?? null,
      clickedEdge: (clickedEdge as any)?.id ?? null
    });
 
    // In link mode, suppress normal node selection to avoid interference
    if (linkModeRef.current && clickedNode) {
      debugLog('🔗 LINK_CREATION_FIX: Link mode active - use right-click for link creation');
      // Suppress node selection in link mode - user should right-click
      return;
    } else {
      debugLog('🔗 LINK_CREATION_FIX: Not in link mode or no node clicked - closing menus');
      // Close any open menus on click (only when not in link creation flow)
      setContextMenu(prev => ({ ...prev, visible: false }));
      setLinkCreateUI(prev => ({ ...prev, visible: false }));
      
      if (clickedNode) {
        debugLog('🔗 LINK_CREATION_FIX: Normal node selection - clickDetail:', event.detail, 'linkMode:', linkMode);
        ...
        if (event.detail === 2 && !linkMode) {
          debugLog('🔗 LINK_CREATION_FIX: Double-click detected - calling onNodeFocus to center view');
        ...
        debugLog('🔗 LINK_CREATION_FIX: Empty space clicked - reset link mode');
      }
    }
  }, [onNodeSelect, onNodeFocus]);

  // Handle right-click context menu for edges and link creation
  const handleCanvasContextMenu = useCallback((event: MouseEvent) => {
    ...
    debugLog('🔗 LINK_CREATION_FIX: Right-click context menu - linkMode:', linkModeRef.current, 'linkCreationState:', linkCreationStateRef.current, 'clickedNode:', clickedNode?.id);

    // Link creation mode: right-click on node
    if (linkModeRef.current && clickedNode) {
      if (!linkCreationStateRef.current.sourceNodeId) {
        // First node - set as source and show confirmation menu
        debugLog('🔗 LINK_CREATION_FIX: Right-click - selecting source node:', clickedNode.id);
        setLinkCreationState({
        ...
        return;
      } else if (clickedNode.id !== linkCreationStateRef.current.sourceNodeId) {
        // Second node - show link creation menu
        debugLog('🔗 LINK_CREATION_FIX: Right-click - creating link from', linkCreationStateRef.current.sourceNodeId, 'to', clickedNode.id);
        setContextMenu({
          visible: true,
          x: event.clientX,
          y: event.clientY,
          ...
          edges: null,
          linkMode: true,
          sourceNodeId: linkCreationStateRef.current.sourceNodeId,
          targetNodeId: clickedNode.id
        });
        return;
      } else {
        // Same node - cancel link creation
        debugLog('🔗 LINK_CREATION_FIX: Right-click - same node clicked, canceling link creation');
        setLinkCreationState({
          sourceNodeId: null,
          isCreatingLink: false
        });
    ...
    // Regular edge/node context menu (not in link mode)
    ...
    // Nothing actionable or cancel; close any existing menu
    setContextMenu(prev => ({ ...prev, visible: false, edges: null, linkId: null, linkData: null, linkMode: false, sourceNodeId: null, targetNodeId: null }));
  }, [linkMode, linkCreationState, getMousePosition, findNodeAtGraphPosition, setLinkCreationState, setContextMenu, setEditRelType]);
  ...
  return (
    <div ref={containerRef} className="absolute inset-0 overflow-hidden bg-gray-50 dark:bg-gray-900">
      ...
      <LegendOverlay />
      {/* Context Menu for Links and Link Creation */}
      <LinkContextMenu
