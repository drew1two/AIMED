...
export class ConportClient {
  ...
  constructor(options: ConportClientOptions = {}) {
    ...
    debugLog('🔧 DEBUG ConportClient constructor completed:', {
  ...
  private async initializeWorkspace() {
    try {
      debugLog('🔧 DEBUG initializeWorkspace starting:', {
        currentWorkspaceId: this.workspaceId,
        timestamp: Date.now()
      });
      
      // Always attempt to get the latest workspace_id from API, even if we think we have one
      const detectedWorkspace = await WorkspaceManager.getDetected();
      debugLog('🔧 DEBUG WorkspaceManager.getDetected() returned:', detectedWorkspace);
      
      if (detectedWorkspace) {
        const normalizedDetected = this.normalizeWorkspaceId(detectedWorkspace);
        if (normalizedDetected !== this.workspaceId) {
          debugLog('🔧 DEBUG Updating workspace_id:', {
            old: this.workspaceId,
            new: normalizedDetected,
            rawDetected: detectedWorkspace
          });
          this.workspaceId = normalizedDetected;
          WorkspaceManager.set(normalizedDetected);
        }
        
        // Now try to get ui-cache environment variables
        try {
          const envVars = await discoverWorkspaceEnvVars(this.workspaceId);
          debugLog('🔧 DEBUG discoverWorkspaceEnvVars returned:', envVars);
          
          // Update workspace_id with authoritative value from ui-cache (should match)
          const normalizedEnvWorkspace = this.normalizeWorkspaceId(envVars.workspace_id);
          if (envVars.workspace_id && normalizedEnvWorkspace !== this.workspaceId) {
            debugLog('🔧 DEBUG Updated workspace_id from ui-cache:', {
              old: this.workspaceId,
              new: normalizedEnvWorkspace,
              rawFromEnv: envVars.workspace_id
            });
            this.workspaceId = normalizedEnvWorkspace;
            WorkspaceManager.set(normalizedEnvWorkspace);
          }
          ...
          if (envVars.mcp_server_url && envVars.mcp_server_url !== this.baseURL) {
            debugLog('🔧 DEBUG Updated MCP server URL from ui-cache:', {
              old: this.baseURL,
          ...
          debugWarn('🔧 DEBUG Failed to discover workspace environment variables:', error);
        }
      } else {
        debugWarn('🔧 DEBUG No workspace_id available after detection');
      }
      
      debugLog('🔧 DEBUG initializeWorkspace completed:', {
        finalWorkspaceId: this.workspaceId,
        finalBaseURL: this.baseURL,
        timestamp: Date.now()
      });
      
      // CRITICAL FIX: Force update the singleton state logging for debugging
      if (defaultClient === this) {
        debugLog('🔧 DEBUG Singleton workspace updated to:', {
          workspaceId: this.workspaceId,
          baseURL: this.baseURL,
          timestamp: Date.now()
        });
      ...
      debugWarn('🔧 DEBUG Failed to initialize workspace:', error);
    }
  }
  ...
  async call<T extends ToolName>(
    ...
    // CRITICAL FIX: Always ensure workspace initialization completes FIRST
    if (typeof window !== "undefined") {
      await this.initializeWorkspace();
    }

    // DEBUG: Track all MCP calls to diagnose Windows ID issue
    debugLog('🔧 DEBUG MCP call start (after workspace init):', {
      toolName,
      args,
      workspaceId: this.workspaceId,
      baseURL: this.baseURL,
    ...
    // DEBUG: Track final validated args before sending
    debugLog('🔧 DEBUG MCP call validated args:', {
      toolName,
      validatedArgs,
      finalWorkspaceId: this.workspaceId
    });
    ...
    // DEBUG: Track final MCP request
    debugLog('🔧 DEBUG MCP request being sent:', {
      request,
      baseURL: this.baseURL,
      sessionId
    });
  ...
  async getProgressById(progressId: number) {
    const allProgress = await this.getProgress({ limit: 1000 }); // Get ALL progress items (high limit to bypass backend defaults)
  ...
  async getDecisionById(decisionId: number) {
    const allDecisions = await this.getDecisions({ limit: 1000 }); // Get ALL decisions (high limit to bypass backend defaults)
  ...
  async getSystemPatternById(patternId: number) {
    const allPatterns = await this.getSystemPatterns({ limit: 1000 }); // Get ALL patterns (high limit to bypass backend defaults)
  ...
  async getLinkedItems(itemType: string, itemId: string, options: any = {}) {
    // DEBUG: Track API calls to diagnose Windows multiple ID issue
    debugLog('🔧 DEBUG getLinkedItems called:', {
      itemType,
      itemId,
    ...
    try {
      debugLog('🔧 DEBUG getLinkedItems - about to call MCP');
      ...
      debugLog('🔧 DEBUG getLinkedItems - MCP call completed');
      
      // DEBUG: Track API response with detailed structure analysis
      debugLog('🔧 DEBUG getLinkedItems result:', {
        itemType,
        itemId,
        ...
        resultResultIsArray: Array.isArray(result?.result),
        resultLength: Array.isArray(result) ? result.length : (result?.result?.length || 'N/A'),
        fullResult: result
      });
      ...
      if (result && typeof result === 'object' && !Array.isArray(result)) {
        if (result.result && Array.isArray(result.result)) {
          debugLog('🔧 DEBUG Returning result.result:', result.result);
          return result.result;
        }
      }
      ...
      // Fallback if structure is different
      if (Array.isArray(result)) {
        debugLog('🔧 DEBUG Returning direct array result:', result);
        return result;
      }
      
      debugLog('🔧 DEBUG Falling back to empty array - result structure unrecognized');
      return [];
    } catch (error) {
      debugWarn('🔧 DEBUG getLinkedItems ERROR:', {
        itemType,
        itemId,
        error,
  ...
  async getAllCustomDataByIdDesc(limit?: number) {
    const result = await this.call("get_all_custom_data_by_id_desc", {
      workspace_id: this.workspaceId,
      limit
    });
...
export function getConportClient(options?: ConportClientOptions): ConportClient {
  // DEBUG: Track singleton usage to diagnose Windows ID contamination
  debugLog('🔧 DEBUG getConportClient called:', {
    hasExistingClient: !!defaultClient,
    existingClientWorkspaceId: defaultClient?.['workspaceId'],
    existingClientBaseURL: defaultClient?.['baseURL'],
    requestedOptions: options,
    timestamp: Date.now()
  });

  if (!defaultClient) {
    debugLog('🔧 DEBUG Creating new ConportClient singleton');
    defaultClient = new ConportClient(options);
  } else {
    debugLog('🔧 DEBUG Reusing existing ConportClient singleton');
  }
  
  // DEBUG: Log client state after creation/reuse
  debugLog('🔧 DEBUG ConportClient state:', {
    workspaceId: (defaultClient as any).workspaceId,
    baseURL: (defaultClient as any).baseURL,
    sessionId: (defaultClient as any).sessionId
  });
...
export function resetConportClient(): void {
  debugLog('🔧 DEBUG resetConportClient called - clearing singleton');
  defaultClient = null;
}
