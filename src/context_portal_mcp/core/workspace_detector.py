...
class WorkspaceDetector:
    ...
    def find_workspace_root(self) -> Path:
        ...
        # Strategy 3: Look for context_portal_aimed directory (existing ConPort workspace)
        workspace = self._detect_by_context_portal()
        if workspace:
            log.info(f"Workspace detected by existing context_portal_aimed: {workspace}")
            return workspace
    ...
    def _detect_by_context_portal(self) -> Optional[Path]:
        """Detect workspace by looking for existing context_portal_aimed directory."""
        current = self.start_path
        depth = 0
        
        while current != current.parent and depth < self.max_depth:
            context_portal_aimed = current / 'context_portal_aimed'
            if context_portal_aimed.exists() and context_portal_aimed.is_dir():
                log.debug(f"Found existing context_portal_aimed at: {current}")
                return current
    ...
    def get_context_portal_path(self, workspace_root: Path) -> Path:
        """Get the context_portal_aimed path for the workspace."""
        return workspace_root / 'context_portal_aimed'
    ...
    def get_detection_info(self) -> Dict[str, Any]:
        ...
        elif (workspace_root / 'context_portal_aimed').exists():
            info['detection_method'] = 'existing_existing_context_portal_aimed'
