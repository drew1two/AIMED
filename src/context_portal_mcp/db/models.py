...
class UpdateDecisionArgs(IntCoercionMixin, BaseArgs):
    """Arguments for updating an existing decision."""
    INT_FIELDS: ClassVar[Set[str]] = {"decision_id"}
    decision_id: int = Field(..., description="The ID of the decision to update.")
    summary: Optional[str] = Field(None, min_length=1, description="New summary for the decision")
    rationale: Optional[str] = Field(None, description="New rationale for the decision")
    implementation_details: Optional[str] = Field(None, description="New implementation details for the decision")
    tags: Optional[List[str]] = Field(None, description="New tags for the decision")

    @model_validator(mode='before')
    @classmethod
    def check_at_least_one_field(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        summary, rationale, implementation_details, tags = values.get('summary'), values.get('rationale'), values.get('implementation_details'), values.get('tags')
        if summary is None and rationale is None and implementation_details is None and tags is None:
            raise ValueError("At least one field ('summary', 'rationale', 'implementation_details', or 'tags') must be provided for update.")
        return values

    @model_validator(mode='after')
    def check_id(self) -> 'UpdateDecisionArgs':
        if self.decision_id < 1:
            raise ValueError("decision_id must be greater than or equal to 1")
        return self

class SearchProgressArgs(IntCoercionMixin, BaseArgs):
    """Arguments for searching progress entries using FTS."""
    INT_FIELDS: ClassVar[Set[str]] = {"limit"}
    query_term: str = Field(..., min_length=1, description="The term to search for in progress entries.")
    limit: Optional[int] = Field(10, description="Maximum number of search results to return.")

    @model_validator(mode='after')
    def check_limit(self) -> 'SearchProgressArgs':
        if self.limit is not None and self.limit < 1:
            raise ValueError("limit must be greater than or equal to 1")
        return self
...
class SearchSystemPatternsArgs(IntCoercionMixin, BaseArgs):
    """Arguments for searching system patterns using FTS."""
    INT_FIELDS: ClassVar[Set[str]] = {"limit"}
    query_term: str = Field(..., min_length=1, description="The term to search for in system patterns.")
    limit: Optional[int] = Field(10, description="Maximum number of search results to return.")

    @model_validator(mode='after')
    def check_limit(self) -> 'SearchSystemPatternsArgs':
        if self.limit is not None and self.limit < 1:
            raise ValueError("limit must be greater than or equal to 1")
        return self
...
class GetAllCustomDataByIdDescArgs(IntCoercionMixin, BaseArgs):
    """Arguments for retrieving all custom data sorted by ID descending (for UI display)."""
    INT_FIELDS: ClassVar[Set[str]] = {"limit"}
    limit: Optional[int] = Field(None, description="Maximum number of entries to return (most recent first)")

    @model_validator(mode='after')
    def check_limit(self) -> 'GetAllCustomDataByIdDescArgs':
        if self.limit is not None and self.limit < 1:
            raise ValueError("limit must be greater than or equal to 1")
        return self
...
class SearchContextArgs(IntCoercionMixin, BaseArgs):
    """Arguments for searching product and active contexts using FTS."""
    INT_FIELDS: ClassVar[Set[str]] = {"limit"}
    query_term: str = Field(..., min_length=1, description="The term to search for in contexts.")
    context_type_filter: Optional[str] = Field(None, description="Optional: Filter by context type ('product' or 'active')")
    limit: Optional[int] = Field(10, description="Maximum number of search results to return.")
    
    @model_validator(mode='after')
    def check_limit(self) -> 'SearchContextArgs':
        if self.limit is not None and self.limit < 1:
            raise ValueError("limit must be greater than or equal to 1")
        return self
    ...
    @model_validator(mode='before')
    @classmethod
    def check_context_type(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        context_type = values.get('context_type_filter')
        if context_type and context_type not in ['product', 'active']:
            raise ValueError("context_type_filter must be 'product' or 'active' if provided")
        return values
...
class UpdateLinkArgs(BaseArgs):
    """Arguments for updating an existing link by ID."""
    link_id: int = Field(..., ge=1, description="The ID of the link to update")
    relationship_type: Optional[str] = Field(None, description="New relationship type")
    description: Optional[str] = Field(None, description="New description for the link")

    @model_validator(mode='before')
    @classmethod
    def check_at_least_one_field(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        if values.get('relationship_type') is None and values.get('description') is None:
            raise ValueError("At least one of 'relationship_type' or 'description' must be provided for update.")
        return values

class DeleteLinkByIdArgs(BaseArgs):
    """Arguments for deleting a link by its ID."""
    link_id: int = Field(..., ge=1, description="The ID of the link to delete")
...
# --- Get Items By References Tool ---

class ItemReference(BaseModel):
    """A reference to a specific ConPort item by type and ID."""
    type: str = Field(..., description="Type of the item (e.g., 'decision', 'progress_entry', 'system_pattern', 'custom_data', 'product_context', 'active_context')")
    id: str = Field(..., description="ID of the item (string to accommodate custom_data keys)")

class GetItemsByReferencesArgs(BaseArgs):
    """Arguments for retrieving multiple items by type/id references."""
    references: Optional[List[ItemReference]] = Field(None, description="List of item type/id pairs to retrieve")
    linked_items_result: Optional[List[Dict[str, Any]]] = Field(None, description="Result from get_linked_items to parse references from")
    include_source_items: bool = Field(True, description="When parsing linked_items_result, include source items")
    include_target_items: bool = Field(True, description="When parsing linked_items_result, include target items")
    
    @model_validator(mode='before')
    @classmethod
    def check_references_or_linked_items(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        references = values.get('references')
        linked_items_result = values.get('linked_items_result')
        if not references and not linked_items_result:
            raise ValueError("Either 'references' or 'linked_items_result' must be provided.")
        if references and linked_items_result:
            raise ValueError("Provide either 'references' or 'linked_items_result', not both.")
        return values
...
TOOL_ARG_MODELS = {
    ...
    "search_context_fts": SearchContextArgs,
    ...
    "get_all_custom_data_by_id_desc": GetAllCustomDataByIdDescArgs,
    ...
    "get_items_by_references": GetItemsByReferencesArgs,
    ...
    "update_link": UpdateLinkArgs,
    "delete_link_by_id": DeleteLinkByIdArgs,
}