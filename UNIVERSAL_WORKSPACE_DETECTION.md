...
3. **Existing ConPort Workspace**  
   A `context_portal_aimed/` directory (database or prior usage) signals a valid root.
...
- `context_portal_path`
- `detection_method`  
  (`strong_indicators` | `multiple_indicators` | `existing_context_portal_aimed` | `fallback`)
- `indicators_found`
